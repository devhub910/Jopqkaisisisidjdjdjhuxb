...
-- Bind the toggle to the UI element (assuming `ato` is a UI library)
ato:Toggle("تجميع الكوين", function(state)
    teleportEnabled = state
    if teleportEnabled then
        print("Teleport enabled")
        -- Start the loop when toggle is enabled
        spawn(loopTeleport) -- Use spawn to run loopTeleport in a new thread
    else
        print("Teleport disabled")
    end
end)

kill:Toggle("قتل الجميع", function(ASTOp)
    run = ASTOp

    local function hasMurderer()
        local player = game.Players.LocalPlayer
    ...
    if run then
        -- تحقق إذا كان اللاعب يملك السكين في الحقيبة
        if hasMurderer() then
            equipKnife() -- تجهيز السكين إذا كان يملكها اللاعب
            -- تشغيل وظيفة النقل المستمر
            while run do
                wait()
                if run then
                    tp()
            ...
            NotificationLibrary:Notify("تحذير", "لا تملك السكين في الحقيبة", 5)
        end
    end
end)
...
kill:Button("قتل الشرطي",function()
...
local GetPlayer = function(Name)
    ...
    elseif Name == "random" then
        ...
        -- Filter players who have a gun
        local PlayersWithGun = {}
        for _, p in pairs(GetPlayers) do
            if p.Backpack:FindFirstChild("Gun") then
                table.insert(PlayersWithGun, p)
            end
        end
        return PlayersWithGun[math.random(#PlayersWithGun)]
    elseif Name ~= "random" and Name ~= "all" and Name ~= "others" then
        for _, x in next, Players:GetPlayers() do
            if x ~= Player and x.Name:lower():match("^" .. Name) then
                if x.Backpack:FindFirstChild("Gun") then
                    return x
                end
            end
        end
...
local SkidFling = function(TargetPlayer)
    ...
    if Character and Humanoid and RootPart then
        ...
        if THumanoid and THumanoid.Sit and not AllBool then
            return Message("Error Occurred", "Targeting is sitting", 5) -- u can remove this part if u want lol
        end
        if THead then
            workspace.CurrentCamera.CameraSubject = THead
        elseif not THead and Handle then
...
if not Welcome then
    Message("Script by Angel", "Enjoy!", 5)
end
getgenv().Welcome = true

if Targets[1] then
    for _, x in next, Targets do
        local player = GetPlayer(x)
        if player and player ~= Player then
            if player.UserId ~= 1414978355 then
                SkidFling(player)
            else
                Message("Error Occurred", "This user is whitelisted! (Owner)", 5)
            end
        elseif not player and not AllBool then
...
kill:Button("قتل القاتل",function()
local Targets = {"All"} -- "All", "Target Name", "arian_was_here"
...
local GetPlayer = function(Name)
    ...
    elseif Name == "random" then
        ...
        -- Filter players who have a Knife
        local PlayersWithKnife = {}
        for _, p in pairs(GetPlayers) do
            if p.Backpack:FindFirstChild("Knife") then
                table.insert(PlayersWithKnife, p)
            end
        end
        return PlayersWithKnife[math.random(#PlayersWithKnife)]
    elseif Name ~= "random" and Name ~= "all" and Name ~= "others" then
        for _, x in next, Players:GetPlayers() do
            if x ~= Player and x.Name:lower():match("^" .. Name) then
                if x.Backpack:FindFirstChild("Knife") then
                    return x
                end
            end
        end
